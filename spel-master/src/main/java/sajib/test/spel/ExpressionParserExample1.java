...
public class ExpressionParserExample1 {
	public static void main(String[] args) {
		...
		Expression expression = expressionParser.parseExpression("'priya dharShini'");
		String strVal = expression.getValue(String.class);
		System.out.println("1. Literal expression value:\n" + strVal);

		// 2. Method invocation
		expression = expressionParser.parseExpression("'Priya Dharshini'.concat('!!!!!!!')");
		...
		expression = expressionParser.parseExpression(" 2"
				+ " matches '[0-9"
				+ "\\s]'");
		boolVal = expression.getValue(Boolean.class);
